(function(){
  ...
  function positionCard(p){
    ...
    return `<article class="asset-card ${trend}">
      <div class="asset-head"><div><strong>${p.symbol}</strong><span>${p.name || ""}</span></div><div class="asset-price"><small>現價</small><b>${fmt2(p.last_price)}</b><em>${p.market === "US" ? "🇺🇸" : "🇹🇼"}</em></div></div>
